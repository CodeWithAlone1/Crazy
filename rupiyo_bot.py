...
Install:
    pip install "python-telegram-bot[job-queue]==20.7" pycryptodome requests aiohttp
...
class Utils:
    ...
    def format_day(dt: datetime) -> str:
        """Cross-platform day formatting without %-d (Windows/Linux safe)."""
    ...
    def _parse_session_dict(data: Dict, ref: str) -> Optional[Dict]:
        try:
            ...
            if not has_refresh:
                ...
                status = "complete"
            # Cross-platform date formatting (replaces %-d which is Linux-only)
            timestamp_str = Utils.format_day(datetime.fromisoformat(ts_str)) if ts_str else 'Unknown'
            return {
                'file':         ref,
                'phone':        phone,
...
class SelfHealer:
    ...
    def sync_profile_fields(device: 'DeviceIdentity', session_file: str):
        try:
            ...
            log.warning(f"sync_profile_fields failed: {e}")
...
_user_offer_cache: Dict[int, Dict] = {}
...
async def cb_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not st.device:
        await safe_edit(query, "❌ Session expired. Use /start to begin a new session.")
        return
    touch(st)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HEALTH SERVER  (Render requires an HTTP server on $PORT)
# ═══════════════════════════════════════════════════════════════════════════════

async def health_handler(request) -> "aiohttp.web.Response":
...
async def start_web_server() -> None:
    ...
    try:
        ...
        if e.errno == 98:
            log.warning(f"Port {PORT} busy — health server skipped")
        else:
            raise
...
def _build_error_report(update: object, err: Exception) -> str:
    ...
    if update and hasattr(update, "effective_user") and update.effective_user:
        ...
        user_line = html.escape(f"{u.full_name} (id={u.id}, @{u.username or 'none'})")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(err, TBadRequest):
        log.warning(f"Telegram BadRequest: {err}")
        return
    if isinstance(err, (TimedOut, NetworkError)):
        log.debug(f"Transient network error: {err}")
        return
    ...
    try:
        if update and hasattr(update, "callback_query") and update.callback_query:
            await update.callback_query.answer("⚠️ An error occurred. Please contact support.", show_alert=True)
            ...
            await update.effective_message.reply_text(user_msg, parse_mode=ParseMode.HTML)
